...
CUDA_SAFE_CALL(hipFree(d_gap_pairs.end_));
    ...
    // output_tpm(h_bins, tpmStore, tpmCount, readCount, nj_readCount, all_readCount, output_dir, file_prefix);
}
...
int main(int argc, char **argv)
{
    ...
    int nprocs, myrank;
    MPI_Status status;
    char *filelist, str[128];
    int err, rank, i;
    double loadtime, runtime;

    MPI_Init (&argc, &argv);
    MPI_Comm_size (MPI_COMM_WORLD, &nprocs);
    MPI_Comm_rank (MPI_COMM_WORLD, &myrank);

    const char* const short_opts = "b:l:x:y:z:R:N:r:o:t:p:q:m:h";
    const option long_opts[] = {
        ...
        {"list", required_argument, nullptr, 'R'},
    ...
    struct GlobalArgs {
        ...
        GlobalArgs(): bin_file(nullptr), length_file(nullptr), 
                      ...
                      a5ss_file(nullptr), file_list(nullptr), file_num(0), read_file(nullptr), 
    ...
    while ((c = getopt_long(argc, argv, short_opts, long_opts, nullptr)) >= 0) {
        ...
        case 'R': ga.file_list = (char *)optarg; break;
        case 'N': ga.file_num = std::stoi(optarg); break;
    ...
    if (ga.file_num == 0) {
      ...
      // check input files
      if (!ga.bin_file || !ga.length_file || !ga.file_list ||
          !ga.se_file || !ga.a3ss_file || !ga.a5ss_file) {
          fprintf(stderr, "[paean] require gene annotation file, length file, ase files and read file!\n");
          exit(EXIT_FAILURE);
      }
...
*/
    ...
    filelist = (char *) malloc (sizeof (char) * 128 * ga.file_num);
    if ((file = fopen (ga.file_list, "r")) == NULL) {
      printf("open %s failed.", ga.file_list);
      exit(EXIT_FAILURE);  
    }
    for (i = 0; i < ga.file_num; i++) {
      err = fscanf (file, "%s", str);
      strcpy (filelist + 128 * i, str);
    }
    fclose (file);
    ...
    if (myrank == 0) {
      for (i = nprocs - 1; i < ga.file_num + nprocs - 1; i++) {
        MPI_Recv (&rank, 1, MPI_INT, MPI_ANY_SOURCE, 99, MPI_COMM_WORLD, &status);
        MPI_Send (&i, 1, MPI_INT, rank, 999, MPI_COMM_WORLD);
      }
    }
    else {
    ...
    // extract file prefix
    std::string file_prefix = extractFilePrefix(filelist + 128 * i);
    ...
    // std::cout << "loading reads..." << std::endl;
    LoadReadFromBam(h_reads, h_nj_reads,
                    h_junctions, h_gaps, filelist + 128 * i,
                    ga.num_threads, ga.read_max_gap,
                    ga.mode);
